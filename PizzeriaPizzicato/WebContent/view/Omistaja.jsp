				...
				<div id="apanelcontent">
					...
					<div>
						<form method="post" action="lisaa">
							<label>Pizzannimi</label><input name="nimi"
								placeholder="pizzan nimi" pattern="^\s*([0-9a-zA-Z ]+)\s*$"
								title="Ei erikoismerkkejä" /> <label>Pizzan kuvaus</label><input
								name="kuvaus" placeholder="kuvaus" /> <label>Pizzan
								hinta</label><input name="hinta" placeholder="hinta"
								pattern="^\d+(\.|\,)\d{2}$"
								title="Anna hinta numeroina muodossa x.xx" /> <select multiple
								name="taytteet">
								<c:forEach items="${taytteet}" var="tayte" varStatus="current">
									<label> Täyte ${current.index} </label>
									<option value="${tayte.getTayte_id()}">
										${tayte.getNimi()}</option>
								</c:forEach>
							</select> <br />
											...
											<div class="pizzainfo">
	                 						...
	                 						${tayte.getNimi()}
													...
													<div class="pizzamuokkaa">
														<button type="button"
															onClick="muokkaa('${current.index}'); return false;">
															<b> muokkaa </b>
														</button>
													</div>
												</div>
												<div class="muokkaus hidden">
													<div class="pizzanimi">
														<input class="pizza_input" type="text"
															value="${pizza.getNimi()}" name="nimi" required
															pattern="^\s*([0-9a-zA-Z ]+)\s*$"
															title="Ei erikoismerkkejä" />
													</div>
													<div class="pizzakuvaus">
														<textarea class="pizza_input" name="kuvaus" required>${pizza.getKuvaus()}</textarea>
													</div>
													<div class="pizzahinta">
														<input type="text" class="pizza_input"
															value="${pizza.getHinta()}" name="hinta" required
															pattern="^\d+(\.|\,)\d{2}$"
															title="Anna hinta numeroina muodossa x.xx" />
													</div>
													<div class="pizzatayte">
													...
													<div class="pizzamuokkaa">
														<button type="submit" value="tallenna" class="tallenna">
															Tallenna</button>
														<a href="poista?id=${pizza.getPizza_id()}"> poista </a>
														<button type="button" value="peruuta" class='peruuta'
															onClick="muokkaa('${current.index}'); return false;">
															Peruuta</button>
														<label for="piilossa"> listalla </label> <input
															type="checkbox" value="piilota" name="piilossa"
															id="piilossa" />
													</div>
												</div>
											</div> <input type=hidden name=id value="${pizza.getPizza_id()}"
											class="hidden" />
										</td>
									</tr>
								</table>
