...
public class LisaaTayteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(validate.teksti(nimi) && validate.teksti(alkupera) && validate.teksti(kuvaus) && validate.hinta(request.getParameter("hinta"))){
			...
			if(lisaaTayte(tayte)){
				request.getSession().setAttribute("notification", tayte.getNimi() + " lis‰ttiin!");
			}else{
				request.getSession().setAttribute("error", "T‰yte on jo listalla.");
			}
		}else{
			request.getSession().setAttribute("error", "Virheelliset kent‰t");
		}
